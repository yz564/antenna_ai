...
import torch
from antenna_parameters import PATCH_SIZES, SIXPATCH_TARGETS, six_patch_antenna
...
def check_plot_comparison(dataset: SimulationDataSet, case_id: int):
    targets = six_patch_antenna.targets
    freqs = six_patch_antenna.freqs
    func = AntennaSimulatorFunc(six_patch_antenna)
...
def plot_scores(train_dataset: SimulationDataSet, model_name=None):
    ...
    if model_name:
        ...
        _, viable = classifier(torch.from_numpy(ims).to(dtype=torch.float32))
...
def plot_scores_all_select_positions(train_dataset: SimulationDataSet):
    ...
    for i in range(N):
        ...
        medians.append(np.median(goal_values[0+casenum*i:casenum+casenum*i]))
        prev_medians.append(np.median(goal_values[casenum*i:casenum+casenum*i]))
    plt.plot(range(1,N+1),medians,color='k',marker='o', markersize=12,label='Generated data median')
    plt.plot(range(1,N+1),prev_medians[:N],color='r',marker='^',markersize=12,label='Classifier threshold')
    for index in range(N):
        ax.text(index+1, medians[index], "{:.1f}".format(medians[index]), size=38, ha='center', va='top')
        ax.text(index+1, prev_medians[index],"{:.1f}".format(prev_medians[index]), size=38, color='r', ha='center', va='bottom')
    plt.ylim((0,6))
    ...
    plt.tight_layout()
    #plt.show()
    #plt.savefig(f"example2_select_positions_points")
    plt.savefig(f"example2_select_positions_points.eps",format='eps')
    plt.close()
    ...
    for i in range(N):
        ...
        handles.append(line)
    plt.scatter([250,250,250,250,250],medians,color='k',marker='o', s=50, zorder=2)
    '''
    for index in range(N):
    ...
    #plt.grid()
    #plt.show()
    #plt.savefig(f"example2_select_positions_curves")
    plt.savefig(f"example2_select_positions_curves.eps",format='eps')
    plt.close()
    ...
    #plt.grid()
    #plt.show()
    #plt.savefig(f"example2_histogram_initial_500")
    plt.savefig(f"example2_histogram_initial_500.eps",format='eps')
    plt.close()
    ...
    plt.tight_layout()
    #plt.show()
    #plt.savefig(f"example2_histogram_last_500")
    plt.savefig(f"example2_histogram_last_500.eps",format='eps')
    plt.close()
...
def plot_scores_all_select_dimensions(train_dataset: SimulationDataSet):
    ...
    '''
    for index in range(N):
        ax.text(index+1, medians[index], "{:.2f}".format(medians[index]), size=38,  ha='center', va='top')
    '''
    for index, median in enumerate(medians):
        label_text = "{:.2f}".format(median)
        ax.annotate(label_text, (index + 1, median), textcoords="offset points", xytext=(0,-40), ha='center', size=38)
    plt.scatter(range(1,N+1),medians, s=400,color='k', marker='*', zorder=2, label='Median')
    plt.ylim((0,6))
    plt.xticks(range(1,N+1),["1","2","3","4","5","6*","6'"])
    plt.xlabel('Dimension set [i]')
    plt.ylabel('Score')
    plt.legend(loc=4, fontsize=32, frameon=False)
    plt.tight_layout()
    #plt.show()
    #plt.savefig(f"example2_select_dimensions_points")
    plt.savefig(f"example2_select_dimensions_points.eps",format='eps')
    plt.close()
    ...
    for i in range(N):
        ...
        simulated_scores=this_set_values[ids]
        if i==5:
            plt.plot(range(len(ids)),simulated_scores,label=f'set i={i+1}*',linewidth=6)
        elif i==6:
            plt.plot(range(len(ids)),simulated_scores,label=f'set i={i}\'',linewidth=6)
        else:
    ...
    plt.legend(loc=4, fontsize=32, ncol=2, frameon=False)
    plt.tight_layout()
    plt.scatter([50,50,50,50,50,50,50],medians,s=400,color='k', marker='*', zorder=2)
    #plt.show()
    #plt.savefig(f"example2_select_dimensions_curves")
    plt.savefig(f"example2_select_dimensions_curves.eps",format='eps')
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--training_dataset', '-f', help='training dataset file', default="training_2500.csv")
    parser.add_argument('--id', '-i',type=int, help='id of data to check', default=0)
    parser.add_argument('--number', '-n',type=int, help='number of data to check', default=3)
    args = parser.parse_args()

    base_dir = os.path.dirname(os.path.abspath(__file__))
    train_data_file = os.path.join(base_dir, "data", args.training_dataset)

    train_dataset = SimulationDataSet(train_data_file, six_patch_antenna)
    goal_values=train_dataset.values
    ...
    #plot_scores(train_dataset, args.model_name)
    #plot_scores_all(train_dataset)
    
    #plot_scores_all_select_positions(train_dataset)  # python3 query_dataset.py -f training_2500.csv
