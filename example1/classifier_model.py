...
class CoordConvLayer(nn.Module):
    """A CNN"""
    def __init__(self, input_channels, output_channels, kernel_size, stride, height, width, padding=0):
        ...
        #self.layer = nn.Conv2d(input_channels , output_channels, kernel_size=kernel_size, stride=stride, padding=padding, dtype=torch.float32)
...
class ConvModel(nn.Module):
    ...
    def __init__(self, pred_space: int, hidden_size: int, backbone_file: str=None):
        ...
        if backbone_file:
            ...
            self.freeze = False
            encoder = []
            encoder.append(CoordConvLayer(1, 64, 3, 1, 60, 300, padding=1))
            ...
            encoder.append(nn.Linear(11200, hidden_size))
